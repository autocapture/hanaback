...
public interface AiPhoneRepairRepository extends JpaRepository<AiPhoneRepair, Long>{
    @Query("SELECT k FROM AiPhoneRepair k WHERE k.accrNo = ?1 AND k.dmSeqno = ?2")
    List<AiPhoneRepair> findByKey(String accrNo, String dmSeqno);

    @Query("SELECT k FROM AiPhoneRepair k WHERE k.accrNo = ?1 AND k.dmSeqno = ?2 AND k.fileName = ?3")
    List<AiPhoneRepair> findByKeyAndFileName(String accrNo, String dmSeqno, String fileName);

    @Query("SELECT k FROM PhoneRepair k WHERE k.fileName = ?1")
    List<AiPhoneRepair> findByFileName(String fileName);
