...
@Getter
@Component
public class MddgDiagDateRule extends ClaimRule {
    ...
    public void setItemInfo(DocType docType){
        initItem(ItemType.MDDG_DIAG_DATE);
    }
    ...
    public void applyRule(List<ValueBox> boxList, List<String> rows, String labelString){
        ...
        String diagDate = "";
        List<String> identifier = cacheService.getDetailKeywords().get(ItemType.MDDG_DIAG_DATE);
...
//                        receiveDate = timeFormatter(expDateRaw);
                        diagDate = expDateRaw;
                        log.debug("[applyRule] expDate {}", diagDate);

                        break;
                    }
                }
                setItemValue(diagDate);
                setAccuracy(getAccuracy(boxList, diagDate));
