...
import com.aimskr.ac2.kakao.backend.common.config.AutocaptureConfig;
...
public class EmailService {
    ...
    private MimeMessage createAssignMessage(String qaOwner, String receiptNo, String mailAddress)throws Exception{
        ...
        message.addRecipients(RecipientType.TO, mailAddress);//보내는 대상
        message.setSubject(qaOwner + ": Kakao 배정알림");//제목
    ...
    private MimeMessage createErrorMessage(String receiptNo, String mailAddress)throws Exception{
        MimeMessage  message = javaMailSender.createMimeMessage();

        message.addRecipients(RecipientType.TO, mailAddress);//보내는 대상
        message.setSubject("Kakao 오류알림");//제목
    ...
    public void sendErrorAlert(String receiptNo, String mailAddress)throws Exception {
        MimeMessage message = createErrorMessage(receiptNo, mailAddress);
        try{//예외처리
            javaMailSender.send(message);
        }catch(MailException es){
            es.printStackTrace();
